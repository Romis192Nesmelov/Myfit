        ...
        @foreach ($programs as $program)
            <optgroup label="{{ $program->title }}">
                @foreach ($program->trainings as $training)
                    <option value="{{ $training->id }}" {{ (!count($errors) ? $training->id == $selected : $training->id == old('training_id')) ? 'selected' : '' }}>{{ view('_case_numeral_duration_block',['value' => $training->duration])->render().'/'.$training->periodicity }}</option>
                @endforeach
            </optgroup>
        @endforeach
    </select>
