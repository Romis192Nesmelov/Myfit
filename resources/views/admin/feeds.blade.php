                ...
                <table class="table datatable-basic table-items">
                    ...
                    @foreach ($data['feeds'] as $feed)
                        <tr role="row" id="{{ 'feed_'.$feed->id }}">
                            ...
                            <td class="text-center head"><a href="/admin/users?id={{ $feed->user->id }}">{{ $advice->user->name }}</a><br>{{ $feed->created_at }}</td>
                            <td class="text-center"><a href="/admin/feed?id={{ $feed->id }}">@include('admin._cropped_content_block',['croppingContent' => $feed->recipe, 'length' => 300])</a></td>
                            <td class="text-center">@include('admin._cropped_content_block',['croppingContent' => $feed->comment, 'length' => 300])</td>
